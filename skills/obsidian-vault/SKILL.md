...
1. **Файловый бэкап машины.** На `pc-virt` это restic, репозиторий `/var/backups/restic`, ежедневный снапшот `/home/popstas` — там лежит и `.claude/`, и `.obsidian/`, и незакоммиченное. Доступ только под root; `sudo` в `!`-режиме Claude Code не получает tty, поэтому ходи через `ssh root@localhost`. Восстанавливай `restic restore <snap> --target /tmp/... --include <путь>` **в отдельный каталог** и сравнивай, а не поверх рабочего дерева: часть свежих изменений может быть легитимной.
2. **Транскрипты сессий** — `~/.claude/projects/<slug>/*.jsonl`: результаты `Read` содержат полный текст файла на момент чтения. Помогает, когда бэкап старше нужного состояния.
3. **File recovery в Obsidian** — снапшоты в IndexedDB на той машине, где правили; переживают откат файла на диске.
4. **Dangling-объекты git** (`git fsck --lost-found`) — только если файл хоть раз попадал в индекс.
