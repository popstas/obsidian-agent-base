...
| `Log/` | Хронологический журнал: дневные логи `YYYY-MM-DD.md`. |
...
**Корневые файлы по желанию:** `index.md` — точка входа, описание структуры, журнал изменений.
...
```
Запись созвона, weekly-апдейт?       → Log/  (имя: YYYY-MM-DD ...md)
...
Файлы в `Log/` — `YYYY-MM-DD.md` или `YYYY-MM-DD <короткое описание>.md`. Сохраняй формат — он используется для сортировки.
...
**Не используй** относительные markdown-ссылки (`[text](../folder/file.md)`) — они хрупкие. Только `[[wikilinks]]` или абсолютные http-ссылки.
...
Любое движение по задаче (из `tasks.md`, `_TODO/`, или просто по ходу дня) фиксируется в **файле дня** `Log/YYYY-MM-DD.md` (см. скилл `[[worklog]]`).
...
Файл разбит на секции `# Week:` (текущая неделя) и `# Week+` (неделя+), ниже `# Week+` — строка-легенда `> Активные задачи. Будущие: [[tasks-future]].`. Внутри `# Week:` сначала идут завершённые (`- [x]`), потом открытые (`- [ ]` / `- [/]`).
...
1. Имя: `Log/YYYY-MM-DD <тема>.md`
2. Если апдейт большой — упомяни в `index.md` (если он есть и поддерживается вручную).
