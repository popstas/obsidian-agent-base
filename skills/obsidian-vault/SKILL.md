...
2. Позиция по умолчанию — **в начало блока открытых задач секции `# Week:`**: сразу после последней `- [x]` и её подпунктов, перед первой открытой (любой чекбокс, кроме `- [x]`). Если в Week нет завершённых — сразу после заголовка `# Week:`.
3. «На потом» / «неделя+» — в конец `# Week+`, перед строкой-легендой. Явное «в конец» — последней строкой секции `# Week:` (перед заголовком `# Week+`).
