...
|---|---|
| `tasks.md` | Активные задачи. Чекбоксы с `➕ YYYY-MM-DD` (создана) и `✅ YYYY-MM-DD` (закрыта). Подгружается хуком при старте сессии. |
...
- Перенос задачи `tasks-future.md` → `tasks.md`: вырезать строку (с подзадачами) и вставить в `tasks.md`, добавив `➕ YYYY-MM-DD` (дата сегодня — момент, когда задача стала активной).

#### Порядок задач в `tasks.md`

В списке задач сначала идут завершённые (`- [x]`), потом открытые (`- [ ]` / `- [/]`).

1. При закрытии задачи перенеси её строку (вместе с подпунктами-отступами) в верхний блок — сразу **после последней `- [x]`** и до первой открытой задачи. Если завершённых задач ещё нет, вставь её перед первой открытой.
2. Новые задачи по умолчанию добавляются в **конец списка**.
3. Если пользователь просит добавить новую задачу «в начало» — вставляй её **после последней завершённой** (в самое начало блока открытых задач), а не выше всех `- [x]`.
4. Не меняй порядок других задач и не схлопывай пустые строки-разделители между смысловыми группами, если они уже есть в файле.
