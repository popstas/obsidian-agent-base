...
1. Определи, к какому скиллу относится урок. Кандидаты ищи в `.claude/skills/<name>/SKILL.md` (в vault, в том числе симлинки на общий репозиторий) и в `~/.claude/skills/` (глобальные). **Если файл оказался симлинком, правка уходит в общий репозиторий и затронет все vault** — предупреди пользователя и спроси подтверждение до правки.
