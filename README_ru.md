...
Этот репозиторий — сразу три вещи: устанавливаемый плагин для Claude Code, Codex и Cursor; готовый к использованию Obsidian-вольт, который клонируешь и адаптируешь под себя; и источник демо-контента, который удаляется после адаптации. Ниже — режим установки под твою задачу.
...
obsidian-plugins.json    какие Obsidian-плагины ожидает вольт
demo-manifest.json       демо-файлы и их хэши, читает скилл demo-content-delete
```

## Установка

### Как новый вольт (большинство пользователей)
...
этот вольт под меня». Агент следует `INTEGRATION.md`, интервьюирует тебя и в конце
запускает скилл `demo-content-delete`, чтобы убрать демо-контент.
...
> агента обе команды — без них скиллы из вольта не активируются:
> ```
...
обновляются автоматически вместе с marketplace. Вольтовую часть — лестницу задач и
...
У вольта, склонированного как «новый вольт», уже есть `.claude/sync-base.cjs` и скилл
